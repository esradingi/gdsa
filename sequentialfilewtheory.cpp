/*
THEORY:
fstream used for reading & writing;
ofstream class used to write on files;
ifstream used to read from files;
ALL 3 derived from IOSTREAM;
...
ios::ate->if flag set, initial pos set to end of file. else initial pos=start of file.
ios::app->appending mode where contents\output inserted at end of file.
ios::trunc->contents of pre existing file are destroyed and are replaced by new data;

is_open()-> returns bool if file open or not;

obj.close()-> closes file

eof()->returs true for determines end of file

Handling Binary files
----------------------
obj.read((char*)&variable,sizeof(variable));
        |--memoryBlock--| |--sizeOfBlock--|
        buffer to store
        read write data

obj.write((char*)&a,sizeof(a));
obj.read((char*)&b,sizeof(b));
data written from array a and read from array b;
----------------------
Functions to reach any desired position:
seekg()-> get pointer of specific location for reading record
       -> seekg(offset,ref-pos);
seekp()-> get pointer of specific location for writing record
       -> seekp(offset,ref-pos);

reference position(ref-pos): specifies beginning(ios::beg), end(ios::end) or current(ios::cur) position in file.

tellg()-> gives current position of get pointer FOR READING;
tellp()-> gives current position of get pointer FOR WRITING;
...
#include<iostream>
#include<fstream>  //fstream used for reading & writing
#include<stdio.h>
#include<string.h>
using namespace std;
class Student
...
void Seq_File::Create()
	{
		ofstream  File; //ofstream class used to write on files
		Student S;
		int i,n;
		cout<<"\nEnter File Name\t";
		cin>>File_Name;
...
void Seq_File::Remove(int Roll)
	{
		 ifstream File;  //used to read from files
		 ofstream Temp;
		 Student S;
		 int Flag=0;
		 File.open(File_Name);
